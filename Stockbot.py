...
# --- Alpaca bars fetcher ---
def fetch_alpaca_bars(symbols: List[str], timeframe="1Day", start=None, end=None, limit=None):
    client = StockHistoricalDataClient(APCA_KEY, APCA_SECRET)
    if start is None:
        # If limit is set, fetch only recent bars
        if limit is not None:
            # For daily bars, go back 'limit' days
    ...
    bars_df = bars_resp.df.reset_index()
    out = {}
    for sym in symbols:
        df = bars_df[bars_df['symbol']==sym].sort_values('timestamp').rename(columns={"timestamp":"time"})[['time','open','high','low','close','volume']]
        if limit is not None:
            df = df.tail(limit)
