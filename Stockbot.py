...
class MA_HighLow_Pullback_Strategy(Strategy):
    def __init__(self, lookback=20, rsi_period=14, rsi_thresh=45, recovery_window=3, trend_window=5, slope_thresh=0.0):
        ...
        self.pending_break_idx = {}

    def analyze(self, symbol, df):
        if len(df) < self.lookback + self.recovery_window + self.trend_window + 31:
            return None

        highs_ma = sma(df['high'], self.lookback)
        lows_ma = sma(df['low'], self.lookback)
        close_sma5 = sma(df['close'], 5)
        close_sma31 = sma(df['close'], 31)
        r = rsi(df['close'], self.rsi_period)

        # Calculate slope of the 31-day moving average
        slope_31 = (close_sma31.iloc[-1] - close_sma31.iloc[-31]) / 31
        #print(f"{symbol} slope_31: {slope_31:.5f}")

        today_idx = len(df) - 1
        entry_signal = None

        # --- Step 1: Look for break if no break is active for this symbol ---
        if symbol not in self.pending_break_idx:
            if df['close'].iloc[-1] < lows_ma.iloc[-1]:
                # check trend condition within trend_window bars BEFORE today
                start_idx = max(0, today_idx - self.trend_window)
                trend_ok = (close_sma5.iloc[start_idx:today_idx] > highs_ma.iloc[start_idx:today_idx]).any()

                # Include slope check
                if trend_ok and slope_31 > self.slope_thresh:
                    # register break for this symbol
                    self.pending_break_idx[symbol] = today_idx

        # --- Step 2: If a break is active, check for recovery ---
        if symbol in self.pending_break_idx:
            bars_since_break = today_idx - self.pending_break_idx[symbol]

            if bars_since_break <= self.recovery_window:
                recovery = df['close'].iloc[-1] > lows_ma.iloc[-1] * 1.02 # Recovery condition

                rsi_ok = r.iloc[-1] >= self.rsi_thresh

                if recovery and rsi_ok:
                    entry = df['close'].iloc[-1]
                    swing_low = df['low'].iloc[self.pending_break_idx[symbol]:today_idx].min()
                    ...
                    entry_signal = Signal(
                        symbol,
                        "buy",
                        f"MA High/Low Pullback (RSI={r.iloc[-1]:.1f})",
                        entry,
                        stop,
                        target,
                        0
                    ...
                    # reset break for this symbol after entry
                    del self.pending_break_idx[symbol]

            else:
                # recovery window expired, reset for this symbol
                del self.pending_break_idx[symbol]
    ...
    def should_exit(self, pos, df):
        ...
        if len(df) >= 4:
            last3_pct_changes = df['close'].pct_change().iloc[-3:]
